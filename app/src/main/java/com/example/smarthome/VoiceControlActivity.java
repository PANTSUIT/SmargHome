...
public class VoiceControlActivity extends AppCompatActivity implements EventListener {
    ...
    public void resulitHandler(String params)
    {
        String result = null;
        String head = "\"best_result\":\"";
        String tail = "\",\"origin_result\"";
        int start = params.lastIndexOf(head);
        start += head.length();
        int end = params.indexOf(tail);
        ...
        if(result.contains("打开"))
        {
            if(result.contains("卧室"))
            {
                ...
                else if(result.contains("风扇"))
                {
                    try {
                        jsonObject.put("fan", 1);
                    } catch (JSONException ex)
                    {
                        ex.printStackTrace();
                    }
                ...
                else if(result.contains("窗帘"))
                {
                    try {
                        jsonObject.put("curtain", 1);
                    } catch (JSONException ex)
                    {
                        ex.printStackTrace();
                    }
